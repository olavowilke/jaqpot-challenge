...
// ---------- POST /casino/launchGame (client-facing, no HMAC) ----------
...
// ---------- POST /casino/getBalance (HMAC, read-only) ----------
...
// ---------- POST /casino/debit (HMAC, atomic, idempotent) ----------
...
// ---------- POST /casino/credit (HMAC, atomic, idempotent) ----------
...
// ---------- POST /casino/rollback (HMAC, atomic, idempotent) ----------
...
casinoRouter.post(
  ...
  wrap(async (req, res) => {
    ...
    const result = await AppDataSource.transaction(async (manager) => {
      ...
      // Tombstone path: original not found.
      if (!original) {
        const tombPayload = {
          status: 'OK',
          transactionId: input.transactionId,
      ...
      // Reject if a credit already exists for this bet.
...
// ---------- POST /casino/simulateRound (test driver, no HMAC) ----------
