...
export class ProviderCasinos1700000000002 implements MigrationInterface {
  ...
  public async up(q: QueryRunner): Promise<void> {
    ...
    // Seed the JAQPOT casino so existing provider_customers rows can be
    // backfilled and so the simulate flow works out of the box.
    await q.query(`
      INSERT INTO provider_casinos (id, casino_code, name, casino_api_endpoint, casino_secret, is_active)
      VALUES ('66666666-6666-6666-6666-666666666666',
              'JAQPOT', 'Jaqpot Casino',
              'http://localhost:3000', 'dev-casino-secret', TRUE)
      ON CONFLICT (casino_code) DO NOTHING
    `);

    // Migrate provider_customers.casino_code -> casino_id (FK).
    await q.query(`ALTER TABLE provider_customers ADD COLUMN casino_id UUID`);
    await q.query(`
      UPDATE provider_customers pc
      SET casino_id = c.id
      FROM provider_casinos c
      WHERE c.casino_code = pc.casino_code
    `);
    // Any rows whose casino_code has no matching directory entry would block
    // the NOT NULL — fail loudly so we notice during migration rather than
    // silently dropping them.
    await q.query(`ALTER TABLE provider_customers ALTER COLUMN casino_id SET NOT NULL`);
    await q.query(
      `ALTER TABLE provider_customers ADD CONSTRAINT fk_provider_customers_casino FOREIGN KEY (casino_id) REFERENCES provider_casinos(id)`,
    );
    await q.query(`ALTER TABLE provider_customers DROP COLUMN casino_code`);
  }

  public async down(q: QueryRunner): Promise<void> {
    await q.query(`ALTER TABLE provider_customers ADD COLUMN casino_code VARCHAR(64)`);
    await q.query(`
      UPDATE provider_customers pc
      SET casino_code = c.casino_code
      FROM provider_casinos c
      WHERE c.id = pc.casino_id
    `);
    await q.query(`ALTER TABLE provider_customers ALTER COLUMN casino_code SET NOT NULL`);
